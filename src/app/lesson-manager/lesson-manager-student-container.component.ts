...
export class CallanLessonManagerStudentContainerComponent implements OnInit, OnDestroy {
    ...
    handleLessonEventCreate() {
        ...
        this.assignGeneralEvents(this.currentDate, this.currentCourseProgress.primaryTeacher);
    ...
    private setCurrentCourseProgress(courseProgress: CallanCourseProgress) {
        ...
        if (this.currentCourseProgress) {
            if (this.currentCourseProgress.minutesBalance >= this.durationEnumKeyIntoNumber(durationsKeys[0])) {
    ...
    private setCalendarEvents() {
        ...
        if (this.lessonEvents) {
            for (const lessonEvent of this.lessonEvents.filter(item => {
                return item.state === CallanLessonEventStateEnum.PLANNED ||
                    item.state === CallanLessonEventStateEnum.STARTED
            })) {
